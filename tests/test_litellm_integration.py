...
def create_mock_litellm_app():
    ...
    async def mock_openai_chat_completions(request_data: dict[Any, Any] = Body(...)):
        if request_data.get("stream", False):
            ...
            def generate():
                yield from OPENAI_STREAMING_RESPONSE
    ...
    async def mock_anthropic_messages(request_data: dict[Any, Any] = Body(...)):
        if request_data.get("stream", False):
            ...
            def generate():
                yield from ANTHROPIC_STREAMING_RESPONSE
...
class TestLiteLLMIntegration:
    ...
    def test_openai_chat_completions_non_streaming(
        ...
        snapshot.assert_match(
            json.dumps(response_data, indent=2), "openai_chat_completion.json"
        )
    ...
    def test_anthropic_messages_non_streaming(self, client, auth_headers, snapshot):
        ...
        response = client.post(
            "/anthropic/v1/messages", json=payload, headers=auth_headers
        )

        assert response.status_code == 200
        response_data = response.json()
        snapshot.assert_match(
            json.dumps(response_data, indent=2), "anthropic_message.json"
        )
